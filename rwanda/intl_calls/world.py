...
class World:
	def __init__(self):
		...
		for i in range(0,len(shpRecords)):
			...
			if cntry_name in colors:
				if cntry_name.find('Zaire') >= 0:
					cntry_name = 'Democratic Republic of the Congo'
				elif cntry_name.find('Congo') >= 0:
					cntry_name = 'Republic of the Congo'
				if cntry_name in self.nations:
					self.nations[cntry_name].append(PolyCollection([verts], facecolor=colors[cntry_name],edgecolor='black'))
				else:
					self.nations[cntry_name] = [PolyCollection([verts], facecolor=colors[cntry_name],edgecolor='black')]
	...
	def load_new_color_map(self, colors, name):
		for cntry, value in colors.items():
			if cntry in self.nations:
				cntries = []
				if cntry == 'United States': cntries = ['Canada', 'United States']
				elif cntry == 'Russia': cntries = ['Russia', 'Kazakhstan']
				else: cntries = [cntry]
				for ct in cntries:
					for part in self.nations[ct]:
						try:
	...
	def animate(self):
		...
		def get_avg_per_time():
			avgs = {}
			avg_res = self.db.query("SELECT `country`,avg(`in_minutes`) FROM `daily_dist` GROUP BY `country`")
			std_res = self.db.query("SELECT `country`,STDDEV(`in_minutes`) FROM `daily_dist` GROUP BY `country`")
			for entry in avg_res:
				avgs[entry[0]] = [float(entry[1])]
			for entry in std_res:
				avgs[entry[0]].append(entry[1])
			return avgs

		def dates(start, end):
			months = {1:31, 2:28, 3:31, 4:30, 5:31, 6:30, 7:31, 8:31, 9:30, 10:31, 11:30, 12:31}
			dates = []
			month_start_date = start[1]
			for year in range(start[0], end[0]+1):
				for month in range(month_start_date, 13):
					for i in range(1, months[month]+1):
						dates.append(str(year) + '-' + add_zero(month) + '-' + add_zero(i))
				month_start_date = 1
			return dates
		...
		for d in dates((2007,1,1), (2007,8,1)):
			query = "SELECT `date`, `country`, `in_minutes` FROM `daily_dist` WHERE `date`='%s'" % d
			res = self.db.query(query)
			#print query
			colors = {}
			for entry in res:
				date, country, count = entry[0], entry[1], entry[2]
				#print count, avgs[(time, country)]
				print date
				colors[country] = self.generate_color(count+0.01, avgs[country][0]-2*avgs[country][1]+0.01, avgs[country][0]+2*avgs[country][1]+0.01)
			n = '__tmp-%s.png' % d
			text.set_text('%s' % d)
			#print colors
	...
	def generate_color(self, value, minimum, maximum):
		...
		try:
			...
			#val = "".join(["%02x"%_interpolate(255, 0, value, minimum, maximum),\
			#"%02x"%_interpolate(255, 0, value, minimum, maximum), "%02x"%_interpolate(255, 0, value, minimum, maximum)])
			#print val, value, minimum, maximum #, _interpolate(155, 0, value, minimum, maximum)
			return '#' + val
		except: return '#ff3366'
